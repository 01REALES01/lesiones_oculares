...
Nuestra plataforma permite **cargar retinografías** (imágenes de fondo de ojo), **ejecutar inferencias concurrentes** utilizando tres modelos de IA (DenseNet169, ResNet50, EfficientNetB0) y **obtener resultados** con métricas, trazabilidad e interpretación orientada a **apoyo clínico y educativo para Retinopatía Diabética (RD)**. **No sustituye el diagnóstico médico.**
...
- **Analizar imagen:** `POST /analyze-retina/`  
  ...
  - Parámetros: `models=densenet169,resnet50,efficientnetb0` (opcional; por defecto los tres).  
